...
environment, and the PowerShell entry script automatically stops the IPC server
when the CLI exits.
...
python3 cli/main.py list --json    # list modules as JSON
...
The `--json` flag is helpful for scripts that need machine-readable module lists.

When run in a real terminal the interactive menu shows a colorful ASCII banner along with the version number. Pipeline steps announce themselves and pause for confirmation when the session is interactive.
...
Modules are numbered sequentially—simply enter the number shown next to a module to run it.
Menu shortcuts are grouped for clarity:

- **Maintenance**: `u` update, `e` self‑evolve, `h` self‑heal, `x` self‑test, `n` view notes
- **Training**: `t` retrain, `v` train CVE, `w` reinforce, `o` operator actions
- **Analysis**: `r` results, `s` stats, `p` plan pipeline, `c` chat, `a` report, `g` gather artifacts
- **Utilities**: `i` `<module>` info, `q` quit

Pipeline modules appear in their own section so you can run complete workflows directly from the menu.
...
- `whois_scan` – perform a WHOIS lookup for a domain.
- `theharvester_scan` – gather emails and subdomains with theHarvester.
- `amass_scan` – perform subdomain enumeration using Amass.
- `httpx_scan` – probe HTTP hosts and record status codes, titles, and detected technologies.
...
- `ffuf_scan` – fuzz web content with ffuf.
- `hydra_scan` – brute-force credentials using Hydra.
- `wpscan_scan` – scan WordPress sites using WPScan.
- `nikto_scan` – scan web servers for known vulnerabilities with Nikto.
- `mythic_control` – manage the Mythic C2 framework (offensive).
- `sliver_control` – manage the Sliver C2 framework (offensive).
- `empire_control` – manage the Empire C2 framework (offensive).
- `covenant_control` – manage the Covenant C2 framework (offensive).
...
- `extended_hunt` – run several scanners (subfinder, hakrawler, dirsearch, wpscan, nuclei, gitleaks, trufflehog) in sequence for deeper analysis.
...
- **Sliver** – cross-platform command-and-control framework
- **Empire** – post-exploitation framework for offensive testing
- **Covenant** – .NET C2 framework for advanced operations
...
Set `MINC_HMAC_KEY` to store a SHA256 HMAC alongside the database so any
tampering is detected when results are loaded.
The server verifies the HMAC when starting up and logs a warning if the signature is invalid.
...
Provide `--template my.tpl` to use a custom Markdown snippet for each entry.
Run `python3 cli/main.py purge --limit 50` to keep only the most recent 50
results if the database grows large.
Run `python3 cli/main.py stats` to display a count of stored results along with
module and tag frequencies. The stats output also reports how often each module
found vulnerabilities, exposing per-module success rates.

Run `python3 cli/main.py gather-poc --out_dir artifacts` to export any raw
request and response data from stored results into the specified directory.
...
chroot into that path for extra protection. When chrooting, `MINC_DB_DIR` and
`MINC_DB_SOCKET` should point to locations inside the chroot. The process drops
to the `nobody` user by default; override this with `MINC_DB_USER` or set it to
an empty string to keep root during tests. The directory used for data and the
IPC socket can be overridden with `MINC_DB_DIR` and `MINC_DB_SOCKET`. For tests
or constrained environments, set `MINC_SKIP_NS=1` to avoid creating a network
namespace. Use `sandbox/main_app/run_main.sh` to start the CLI, optionally
dropping privileges by setting `MINC_CLI_USER`.
...
python3 recon_modules/manager.py funcs ping_sweep
python3 recon_modules/manager.py call ping_sweep main 127.0.0.1
python3 recon_modules/manager.py best ping_sweep
python3 recon_modules/manager.py info ping_sweep
python3 vuln_modules/manager.py info sqli_scanner
```

Running modules through these managers automatically logs the parameter list to
the analysis database when `MINC_DB_SOCKET` is set, giving the neural analyzer
more context for future recommendations.

Each result also records simple code metrics – the number of functions and total
lines for the module’s source file – so the neural analyzer can learn from the
structure of the codebase itself when making suggestions.

The `github_scanners/manager.py` script also supports an `info` command that
prints a short description of each upstream tool without executing it. Run

```bash
python3 github_scanners/manager.py info ssrfmap
```

to see details for a scanner. Wrapper modules for new scanners can be created
automatically with:

```bash
python3 scripts/sync_scanner_wrappers.py
```

This ensures newly cloned tools are available to pipelines without manual
coding.

You can also generate the `manager.py` script for a new directory:

```bash
python3 scripts/create_manager.py /path/to/dir --kind module
```
The generated manager lists modules, shows available functions, and can invoke
any function directly, reducing boilerplate.
You can also generate skeleton modules automatically:

```bash
python3 scripts/create_module.py recon new_scan
```
This creates `recon_modules/new_scan.py` with boilerplate code.

To analyze a repository or module for its command-line parameters, use the
`discover_params.py` helper:

```bash
python3 scripts/discover_params.py path/to/module_or_repo
```
This scans Python files for `argparse` definitions and prints any discovered
options so you can quickly understand how to invoke new tools.
...
python3 offensive_modules/manager.py run sliver_control install
python3 offensive_modules/manager.py run empire_control install
python3 offensive_modules/manager.py run covenant_control install
```
...
Use the operator interface to tune the neural model, approve findings,
or pause and resume jobs:

```bash
python3 cli/main.py operator tune 0.7
python3 cli/main.py operator approve FINDING_ID
python3 cli/main.py operator pause JOB_ID
python3 cli/main.py operator resume JOB_ID
```
...
This sequence performs subdomain discovery, crawling, directory brute force, WordPress scanning with WPScan, generic vulnerability scanning and secret detection before saving combined results.
...
```bash
python3 cli/main.py self-evolve [--target <host>] [--heal] [--patch SCRIPT] [--iter N]
```
...
to limit CPU usage. Use `--iter N` to run multiple evolution cycles and gather
more training data. Use `--heal` to reinstall local scanner repositories and
...
After the pipeline completes, ChainHunter queries the neural analyzer for
recommended individual modules and automatically runs the top suggestion to
...
Run the internal test suite to verify installation integrity:

```bash
python3 cli/main.py self-test
```

The command executes `pytest` with `SKIP_CLONE=1` so repositories are not
reinstalled. Use this after updates to ensure everything still works.
...
The project tracks environment checks in `SETUP_VERIFICATION.txt`. Run
`python3 scripts/setup_verifier.py` to list which installation steps are done
or still pending and update the file as the setup is validated.
...
modules are likely to discover additional vulnerabilities. The database now
updates module interaction patterns whenever results are saved, so follow-up
suggestions learn from the sequences of tools that were run previously.

```bash
python3 cli/main.py suggest-mods -n 5
```

The command prints a ranked list of module names. You can ask for follow-up
suggestions after a specific module by using `--after`:

```bash
python3 cli/main.py suggest-mods --after ping_sweep
```

### Parameter recommendations

You can also ask ChainHunter which parameters work best for a module based on
past results:

```bash
python3 cli/main.py suggest-params ping_sweep -n 10
```

This prints the top parameter combinations observed for `ping_sweep`.

You can then automatically run the module with the best parameters using the
manager scripts:

```bash
python3 recon_modules/manager.py best ping_sweep -n 10
```
The manager fetches suggestions from the database and launches the module with
the top recommendation.

### Explore new modules

When adding a new module, you can ask ChatGPT for example commands based on its
documentation:

```bash
python3 cli/main.py explore ping_sweep -n 2
```

The output lists suggested commands to try.
...
### Train with CVE dataset

Provide a JSON file containing CVE-inspired examples to further train the
neural analyzer:

```bash
python3 cli/main.py train-cve data/cve_samples.json
```

### Reinforce with successful cases

Provide a JSON file containing past bug bounty successes with a `reward` or
`vuln_count` field to reinforce the neural analyzer:

```bash
python3 cli/main.py train-success data/success_samples.json
```
The command prints the parameter change (Δ) after applying the reinforcement
data so you can gauge its impact.

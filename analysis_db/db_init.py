...
def handle_scan(result: Dict) -> Dict:
    ...
    try:
        ...
        update_interactions_from_results(data)
    except Exception:
        pass
    return {"status": "ok"}
...
def handle_stats() -> Dict:
    ...
    mods: Dict[str, int] = {}
    tags: Dict[str, int] = {}
    success: Dict[str, Dict[str, float]] = {}
    for entry in data:
        mod = entry.get("module")
        if mod:
            mods[mod] = mods.get(mod, 0) + 1
            info = success.setdefault(mod, {"runs": 0, "vuln": 0, "rate": 0.0})
            info["runs"] += 1
            if entry.get("vuln_count", 0) > 0:
                info["vuln"] += 1
        for t in entry.get("tags") or []:
            tags[t] = tags.get(t, 0) + 1
    for stats in success.values():
        if stats["runs"]:
            stats["rate"] = stats["vuln"] / stats["runs"]
    return {
        "status": "ok",
        "count": len(data),
        "modules": mods,

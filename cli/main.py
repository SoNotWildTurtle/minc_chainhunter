...
def run_module(name: str, args: List[str]) -> bool:
    ...
    try:
        ...
        if '--debug' in sys.argv or '-v' in sys.argv:
            import traceback
            traceback.print_exc()
        if not os.environ.get("MINC_HEALED"):
            try:
                os.environ["MINC_HEALED"] = "1"
                from scripts.self_heal import run_self_heal
                logger.info("Attempting self-heal after failure")
                if run_self_heal():
                    logger.info("Retrying module after self-heal")
                    return run_module(name, args)
            except Exception as heal_err:
                logger.error(f"Self-heal failed: {heal_err}")
        return False
...
def show_interactive_menu() -> None:
    ...
    Returns a list of module names in the order they were displayed so that
    numeric choices line up with the correct module.
    """
    ...
    if recon_modules:
        if sys.stdout.isatty():
            ...
            print("\nReconnaissance:")
        for mod in sorted(recon_modules):
            idx = len(order) + 1
            if sys.stdout.isatty():
                print(GREEN + f"  [{idx}] {mod}" + RESET)
            else:
                print(f"  [{idx}] {mod}")
            order.append(mod)
    ...
    if vuln_modules:
        ...
        for mod in sorted(vuln_modules):
            idx = len(order) + 1
            if sys.stdout.isatty():
                print(GREEN + f"  [{idx}] {mod}" + RESET)
            else:
    ...
    if pipeline_modules:
        if sys.stdout.isatty():
            ...
            print("\nPipelines:")
        for mod in sorted(pipeline_modules):
            idx = len(order) + 1
            if sys.stdout.isatty():
                print(GREEN + f"  [{idx}] {mod}" + RESET)
            else:
    ...
    if sys.stdout.isatty():
        print(PURPLE + "\nEnter the number of a module to run it." + RESET)
    else:
        print("\nEnter the number of a module to run it.")
...
def interactive_mode() -> None:
    ...
    while True:
        try:
            module_list = show_interactive_menu()
            choice = input("\n> ").strip().lower()

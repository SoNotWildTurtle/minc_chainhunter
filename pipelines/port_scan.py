"""Pipeline that performs masscan followed by nmap."""
import importlib
import os
import sys
...
STEPS = [
    ("masscan_scan", "recon_modules.masscan_scan"),
    ("nmap_scan", "vuln_modules.nmap_scan"),
]
...
def main(argv: List[str] | None = None):
    ...
    if not argv:
        print("Usage: port_scan.py <target>")
        return False
    target = argv[0]
    results = []
...
if __name__ == "__main__":
    ...
    if sock and isinstance(summary, dict):
        try:
            log_scan_result(sock, {"module": "port_scan", **summary})
        except Exception:
            pass
    if isinstance(summary, dict) and sys.stdout.isatty():
        print(Fore.MAGENTA + "[+] Port scan pipeline completed" + Style.RESET_ALL)

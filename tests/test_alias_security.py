import os
import sys

sys.path.insert(0, os.path.dirname(os.path.dirname(__file__)))

import ipc_bus.bus_integrity as bi


def test_alias_is_approved():
    assert bi.is_alias_approved("scan")
    assert bi.is_alias_approved("report")
    assert bi.is_alias_approved("results")
    assert bi.is_alias_approved("search")
    assert bi.is_alias_approved("chat")
    assert bi.is_alias_approved("plan")
    assert bi.is_alias_approved("train")
    assert bi.is_alias_approved("train_cve")
    assert bi.is_alias_approved("train_success")
    assert bi.is_alias_approved("operator")
    assert bi.is_alias_approved("explore")
    assert not bi.is_alias_approved("rm -rf")

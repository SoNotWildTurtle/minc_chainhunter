...
def test_bug_hunt_pipeline(tmp_path):
    ...
    proc = subprocess.run(
        [sys.executable, "cli/main.py", "run", "bug_hunt", "127.0.0.1"],
        capture_output=True,
        text=True,
        env=env,
    )
    # server runs in daemon thread; no join needed

...
def test_db_alias_commands(tmp_path):
    ...
    t.start()
    time.sleep(0.1)
    resp = generate_report(str(sock), str(out_dir), "json")
    t.join()
    assert resp["status"] == "ok"
    ...
    t.start()
    time.sleep(0.1)
    resp = generate_report(str(sock), str(out_dir), "pdf")
    t.join()
    assert resp["status"] == "ok"
    assert os.path.isfile(resp["path"])
    ...
    # plan alias should respond
    t = threading.Thread(target=start_db_server, args=(str(db_dir), str(sock), True))
    t.start()
    time.sleep(0.1)
    resp = send_request(str(sock), {"alias": "plan"})
    t.join()
    assert resp["status"] == "ok"
    ...
    # stats alias should respond
    t = threading.Thread(target=start_db_server, args=(str(db_dir), str(sock), True))
    t.start()
    time.sleep(0.1)
    resp = send_request(str(sock), {"alias": "stats"})
    t.join()
    assert resp["status"] == "ok"

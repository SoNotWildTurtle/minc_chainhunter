...
def test_recon_manager_lists_modules(tmp_path):
    ...
    assert 'ffuf_scan' in vuln_proc.stdout
    assert 'hydra_scan' in vuln_proc.stdout
...
def test_manager_run_logs_params(tmp_path):
    repo_root = os.path.dirname(__file__) + '/..'
    sock = tmp_path / 'db.sock'
    data_dir = tmp_path / 'data'
    t = threading.Thread(target=start_db_server, args=(str(data_dir), str(sock), True))
    ...
    with open(data_dir / 'results.json') as f:
        results = json.load(f)
    assert any(r.get('module') == 'dns_lookup' and r.get('params') == ['localhost'] for r in results)
